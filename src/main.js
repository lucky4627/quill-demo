...
class CodeBlock extends Block {
  ...
  delta() {
    let text = this.domNode.querySelector("code").textContent;
    if (text.endsWith("\n")) {
  ...
  length() {
    let length = this.domNode.querySelector("code").textContent.length;
    if (!this.domNode.querySelector("code").textContent.endsWith("\n")) {
      return length + 1;
    }
    return length;
  }
  ...
  newlineIndex(searchIndex, reverse = false) {
    if (!reverse) {
      let offset = this.domNode
        .querySelector("code")
        .textContent.slice(searchIndex)
        .indexOf("\n");
      return offset > -1 ? searchIndex + offset : -1;
    } else {
      return this.domNode
        .querySelector("code")
        .textContent.slice(0, searchIndex)
        .lastIndexOf("\n");
    }
  }
